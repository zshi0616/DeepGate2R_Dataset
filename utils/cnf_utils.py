...
def kissat_solve(iclauses, no_vars, tmp_dir=None, args=None):
    tmp_name = 'tmp_solve_{:}_{:}_{:}_{:}_{:}.cnf'.format(
        datetime.now().hour, datetime.now().minute, datetime.now().second, len(iclauses), random.randint(0, 10000)
    )
    if tmp_dir == None:
        tmp_filename = os.path.join('./tmp', tmp_name)
    else:
...
def solve_with_learnt(solver, iclauses, no_vars, tmp_filename=None, args=None):
    if tmp_filename == None:
        tmp_filename = './tmp/tmp_solve_{:}_{:}_{:}_{:}.cnf'.format(datetime.now().hour, datetime.now().minute,
                                                                    len(iclauses), random.randint(0, 100))
    save_cnf(iclauses, no_vars, tmp_filename)
    if args != None:
        cmd_solve = '{} {} -q {}'.format(solver, args, tmp_filename)
    else:
    ...
    for line in solve_info:
        if 'Size' in line:
            ...
            lits = new_line.split(']')[0].split(',')
            clause = []
            for lit in lits:
                if lit == '':
                    continue
                clause.append(int(lit))
            arrs = new_line.split(']')[-1].split(',')

            learnt_clauses.append(clause)
            learnt_clauses_glue.append(int(arrs[1].split(':')[-1]))
            learnt_clauses_conflicttimes.append(int(arrs[2].split(':')[-1]))
        if '#Dec' in line:
            no_dec = int(line.replace('\n', '').replace(' ', '').split(':')[-1])

    learnt_info = [learnt_clauses, learnt_clauses_glue, learnt_clauses_conflicttimes]
    ...
    return sat_status, asg, learnt_info, solvetime, no_dec
