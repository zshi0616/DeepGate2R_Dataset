...
def cnf_to_xdata(cnf_filename, tmp_aig_filename, tmp_aag_filename, gate_to_index):
    ...
    aig = aiger.load(tmp_aig_filename)
...
# x_data: [ [idx, gatetype, level] ]
def aig_to_xdata_withlevel(aig_filename, gate_to_index={'PI': 0, 'AND': 1, 'NOT': 2}):
    aig = aiger.load(aig_filename)
    f = str(aiger.BoolExpr(aig))
    lines = f.split('\n')
    header = lines[0].strip().split(" ")
    ...
    # PI 
    for i in range(n_inputs):
        x_data.append([len(x_data), gate_to_index['PI'], 0])
    # AND 
    for i in range(n_and):
        x_data.append([len(x_data), gate_to_index['AND'], 0])
    ...
    for (i, line) in enumerate(lines[1+n_inputs+n_outputs: ]):
        ...
        if fanin_1_not == 1:
            if has_not[fanin_1_index] == -1:
                x_data.append([len(x_data), gate_to_index['NOT'], 0])
                not_index = len(x_data) - 1
                edge_index.append([fanin_1_index, not_index])
                has_not[fanin_1_index] = not_index
            fanin_1_index = has_not[fanin_1_index]
    ...
    for (i, line) in enumerate(lines[1+n_inputs: 1+n_inputs+n_outputs]):
        ...
        if po_not == 1:
            if has_not[po_index] == -1:
                ...
                has_not[po_index] = not_index
    ###
    _, fanout_list = circuit_utils.get_fanin_fanout(x_data, edge_index)
    x_data_level = circuit_utils.get_each_level(x_data, fanout_list)
    for idx in range(len(x_data)):
        x_data[idx][2] = x_data_level[idx]
    ###
    return x_data, edge_index

def aig_to_cnf(data, fanin_list, gate_to_index={'INPUT': 0, 'AND': 1, 'NOT': 2}, const_0=[], const_1=[], add_clauses=[]):
    cnf = []
...
def aigcone_to_cnf(data, fanin_list, cone_po, cone_po_val, gate_to_index ={'PI': 0, 'AND': 1, 'NOT': 2}):
    # Mask
    mask = [0] * len(data)
    bfs_q = [cone_po]
    while len(bfs_q) > 0:
...
def aigcones_to_cnf(data, fanin_list, cone_po_list, cone_po_val_list=[], gate_to_index ={'PI': 0, 'AND': 1, 'NOT': 2}):
    # Mask
    mask = [0] * len(data)
    bfs_q = []
    for cone_po in cone_po_list:
        bfs_q.append(cone_po)
    while len(bfs_q) > 0:
        idx = bfs_q[-1]
        mask[idx] = 1
        bfs_q.pop()
    ...
    for cone_po_val in cone_po_val_list:
        if cone_po_val:
            cnf.append([cone_po + 1])
        else:
            cnf.append([-1 * (cone_po + 1)])
